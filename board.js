...
function displayPlayer(playerNo, boxNo) {
    ...
    if (playerNo === 0) {
        ...
        document.getElementById(boxId).innerHTML += `<div class='player one on-board'></div>`; 
    } else {
        let ele = document.querySelector(".two.on-board");
        if (ele) {
            ele.parentNode.removeChild(ele);
        }
        document.getElementById(boxId).innerHTML += `<div class='player two on-board'></div>`;
    }
}
...
//function takes care of enabling and disabling play buttons
